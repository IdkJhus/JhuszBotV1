...
module.exports = class FortniteShop {
  constructor() {
    ...
    this.textFooter = "Generated with Swiftcord - fnbr.co";
    this.options = {
      lang: "en",
      dateFormat: "dddd, MMMM Do YYYY",
    };
    this.background = `${__dirname}/../assets/shop/background.png`;
  }
  ...
  async toAttachment() {
    ...
    const filesDir = `${__dirname}/../assets/shop/cache`;
      ...
      files.forEach(function (file) {
        if (file.split(".")[1] === "png") {
          if (Number(file.split("_")[0]) < Date.now() - 86400000 * 5) {
            console.log(file);
            fs.unlinkSync(
              `${__dirname}/../assets/shop/cache/${file}`
            );
          }
        }
      });
    ...
    const path = `${__dirname}/../assets/shop/cache/${new Date(
      shop.data.date
    ...
    if (fs.existsSync(path)) {
      ...
      if (this.background.startsWith("#")) {
        ...
        let background = await Canvas.loadImage(
          `${__dirname}/../assets/shop/background.png`
        );
        if (canvas.height > 2026) {
          ctx.drawImage(
            background,
