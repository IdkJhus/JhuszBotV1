let handler = async (m, { conn, args, command }) => {
    ...
    if (command.endsWith('all') || command.endsWith('semua')) {
        for (let i = 0; i < chat.length; i++) { // For loops
            await m.reply('Nos vemos 👋, fue bonito mientras duró...', chat[i].jid)
        ...
        if (!ada) throw 'El bot no se encuentra en ese grupo'
        await m.reply('Nos vemos 👋, fue bonito mientras duró...', args[0])
        await conn.groupLeave(args[0])
        await m.reply('Succes!!')
    } else {
        if (!m.isGroup) return global.dfail('group', m, conn)
        await m.reply('Nos vemos 👋, fue bonito mientras duró...', m.chat) // WKWKW pesannya sama semua. gk kreatif :v
